...
- [ ] **Inventory snapshot** – dump a table of current fixtures/helpers and their scopes before moving them; identify pytest markers and shared state.
- [ ] **Module skeletons** – create empty `tests/vm/*.py` files with docstrings describing intended contents; wire `__init__.py` if needed and keep imports passing lint.
- [ ] **Shim plan** – decide whether `tests/vm/conftest.py` or the legacy file re-exports fixtures; document chosen approach and migration steps.
- [ ] **RAID/LVM recipe** – write the exact mdadm/LVM commands that will seed leftover devices inside the VM (include cleanup expectations) and how to assert teardown.
...
- [ ] **Timeout defaults** – codify generous default values for spawn/login timeouts (diagnostic only) and note how to override via env vars; thread through helper constructors.

...
1. **Validate the fix end-to-end in the VM regression.**
   ...
   - 2025-10-16T05-45-00Z - Attempting to rebuild `.#bootImage` with a freshly generated `PRE_NIXOS_ROOT_KEY` stalled: `nix develop .#bootImageTest` aborted because the ISO derivation `/nix/store/26zylzprcx44248ypkg39yk6hp66qcf5-...iso.drv` exited 1 without logs while `mksquashfs` kept running for >15 minutes. Need to capture the failing build log (e.g. rerun with `--keep-failed`) and determine why the impure build dies during squashfs creation before the VM regression can be retried.
    - 2025-10-17T03-10-54Z - Storage provisioning now completes (`STATE=applied`/`DETAIL=auto-applied`), but the VM assertions fail because `vgs --noheadings` runs as the unprivileged `nixos` user (LVM lock permission denied) and the captured `systemctl is-active pre-nixos` output still includes the buffered IPv4 probe. Artefacts in `docs/work-notes/2025-10-17T03-20-16Z-boot-image-vm-regression/`; test summary in `docs/test-reports/2025-10-17T03-10-54Z-boot-image-vm-test.md`.
   - 2025-10-17T18-45-00Z - Rebuilt `.#bootImage` successfully with `PRE_NIXOS_ROOT_KEY` injected (`/nix/store/d4k503px9si7gi51kyn7hfhv4m5wvbjw-nixos-24.05.20241230.b134951-x86_64-linux.iso`) but `TMPDIR=/tmp nix develop .#bootImageTest -c pytest tests/test_boot_image_vm.py -vv` still fails: the session-scoped `nix build .#bootImage --impure --no-link --print-out-paths` exits 1 before returning a store path even though standalone rebuilds succeed. Fixture stderr shows `...-nixos-24.05.20241230.b134951-x86_64-linux.iso.drv failed to build`; manual reruns with `--keep-failed` finish without reproducing the error. Need to capture the failing build log within the pytest workflow or teach the harness to surface the nix build stderr so the underlying cause can be diagnosed. 【de07ce†L1-L38】

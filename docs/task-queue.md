...
1. **Confirm dependent services behave with sshd held back by `wantedBy = []`.**
...
5. **Compare harness and service toggles to isolate any remaining regressions.**
...
6. **Expand unit coverage and regression safeguards.**
...
7. **Harden BootImageVM diagnostics.**
...
8. **Rebuild the boot image with future network/storage tweaks and rerun the VM regression.**
...
9. **Capture follow-up boot timings after configuration adjustments.**
    - With the harness stable, collect new timing data after each substantive change to detect regressions early.

10. **Ensure the full test suite runs without skips.**
